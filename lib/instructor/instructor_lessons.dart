...
class InstructorLessons extends StatefulWidget {
  const InstructorLessons({super.key, this.requests});
  ...
  @override
  State<InstructorLessons> createState() => _InstructorLessonsState();
}

class _InstructorLessonsState extends State<InstructorLessons> {
  ...
  addLesson() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AddLesson(requests: widget.requests),
      ),
    ).then((value) => setState(() {
          pendingLessons = lessons(isPending: true);
          finishedLessons = lessons(isPending: false);
  ...
  Widget build(BuildContext context) {
    if (widget.requests == null || widget.getApproved().isEmpty) {
      return const Center(child: Text('No active trainees to assign lessons.'));
    } else {
      int completed = 0;
      int pending = 0;
      lessonsCount.forEach(
                ...
                child: Scaffold(
                  ...
                  body: IndexedStack(
                    ...
                    children: [
                      //pending
                      pendingLessons == null
                          ? const Center(child: Text('No pending lessons.'))
                          : Scaffold(
                              body: FutureBuilder(
                      ...
                      finishedLessons == null
                          ? const Center(child: Text('No finished lessons.'))
                          : Scaffold(
                              body: FutureBuilder(
            ...
            : ActionPane(
                ...
                children: [
                  SlidableAction(
                    ...
                    icon: Icons.edit,
                    label: 'Edit',
                    onPressed: (context) => editLesson(lesson),
                  ),
                  SlidableAction(
                    ...
                    icon: Icons.delete,
                    label: 'Delete',
                    onPressed: (context) => deleteLesson(lesson),
                  ),
                ],
              ),
        child: ListTile(
