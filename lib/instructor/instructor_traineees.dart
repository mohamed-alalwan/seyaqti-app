...
class InstructorTrainees extends StatefulWidget {
  const InstructorTrainees({super.key, this.requests});
  ...
  @override
  State<InstructorTrainees> createState() => _InstructorTraineesState();
}

class _InstructorTraineesState extends State<InstructorTrainees> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: widget.requests == null || widget.getApproved().isEmpty
          ? const Center(child: Text('No trainees yet.'))
          : ListView.builder(
              itemCount: widget.getApproved().length,
              shrinkWrap: true,
              itemBuilder: (context, index) {
                final requestDoc = widget.getApproved()[index];
                final id = requestDoc.data().senderID!;
                return StreamBuilder(
                  stream: getUserDoc(id).snapshots(),
                  builder: (context, snapshot) {
                    if (snapshot.hasData) {
                      final user = snapshot.data!.data()!;
                      return buildUser(user, requestDoc.id);
                    } else {
