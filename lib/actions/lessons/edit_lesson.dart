...
class EditLesson extends StatefulWidget {
  const EditLesson({super.key, this.requests, required this.lesson});
...
class _EditLessonState extends State<EditLesson> {
  ...
  void initState() {
    super.initState();
    declareTrainees();
    selectedTrainee = widget.lesson.traineeID;
    selectedDate = widget.lesson.date;
    titleController.text = widget.lesson.title!;
    dateController.text = DateFormat.yMEd().format(widget.lesson.date!);
    timeController.text = DateFormat.jm().format(widget.lesson.date!);
    durationController.text = widget.lesson.duration!.toString();
    isPickup = widget.lesson.isPickup!;
  }
  ...
  Future editLesson() async {
    try {
      ...
      Utils.ShowSuccessBar('Lesson successfully modified.');
      if (!mounted) return;
      Navigator.pop(context);
    } on FirebaseException catch (_) {
      Utils.ShowErrorBar('Something went wrong...');
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: isEditingLesson
          ? const Scaffold(body: Loading(color: Colors.white10))
          : Scaffold(
              appBar:
                  AppBar(title: const Text('Edit Lesson'), centerTitle: true),
              body: isLoading
                              ...
                              onTap: () async {
                                TimeOfDay? pickTime = await showTimePicker(
                                  ...
                                  initialTime: TimeOfDay(
                                    hour: widget.lesson.date!.hour,
                                    minute: widget.lesson.date!.minute,
                                  ),
                                );
